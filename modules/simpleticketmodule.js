...
console.log(` :: ⬜️ Module: ${description.name} | Phiên bản đã tải ${description.version} from ("${description.filename}")`.bold.green)
  let disabled = new MessageEmbed()
    .setColor(config.colors.no)
    .setTitle("Chủ sở hữu của bạn đã vô hiệu hóa Hệ thống bán vé ( ticket )! Lấy làm tiếc")
    .setFooter("yêu cầu bởi", config.AVATARURL)  .setColor(config.colors.no)
    ...
    client.on("messageReactionAdd", async (reaction,user) => {
        ...
        client.setups.ensure(reaction.message.guild.id, 
          {
              ...
              channelid: "",
              parentid: "",
              message: "Hey {user}, cảm ơn vì đã mở một vé ( ticket )! Ai đó sẽ giúp bạn sớm!",
              adminroles: []
          }, "ticketsystem"
      );
        ...
        if (reaction.message.guild.id === ticket.guildid && reaction.message.id === ticket.messageid){
          if(!ticket.enabled)  return user.send(disabled).catch(e=>console.log("TICKET:".underline.red + " :: " + e.stack.toString().red));
            ...
            if(alreadyticket) 
                return user.send("Xin lỗi, nhưng bạn có một vé ( ticket ) đã mở!<#" + alreadyticket + ">").catch(e=>console.log("TICKET:".underline.red + " :: " + e.stack.toString().red));

            reaction.message.guild.channels.create(channelname.substr(0,31), { parent: ticket.parentid, topic: `ticket-${user.id}` }).then(ch => {
              ch.updateOverwrite(reaction.message.guild.roles.everyone, {
                  SEND_MESSAGES: false,
                  VIEW_CHANNEL: false,
              });
              ...
              if(reaction.message.guild.roles.cache.some(r=>ticket.adminroles.includes(r.id))){
                  for(let i = 0; i<ticket.adminroles.length; i++){
                        try{
                            ch.updateOverwrite(ticket.adminroles[i], {
                                SEND_MESSAGES: true,
                                VIEW_CHANNEL: true,
                            });
                        }catch (error) {
                ...
                ch.send(ticket.message.replace("{user}", `${user}`)).catch(error => console.log(error)).then(msg => msg.react("🔒")).catch(e=>console.log("TICKET:".underline.red + " :: " + e.stack.toString().red));
            })
        }
        ...
        if(reaction.emoji.name === "🔒"){
            ...
            if (message.channel.parent.id === ticket.parentid) {
              ...
              try {
                ...
                member.send(`Vé ( ticket ) của bạn đã được đóng bởi: \`${member.user.tag}\`! Đây là một bản ghi:`).catch(error => console.log(error)).catch(e=>console.log("TICKET:".underline.red + " :: " + e.stack.toString().red));
        
              } catch (error) {            ///// đã việt hoá đến đây
