    ...
    readdirSync("./commands/").forEach(dir => {
        ...
        for (let file of commands) {
            ...
            if (pull.name) {
                ...
                table.addRow(file, `LỖI -> thiếu cmd.name hoặc cmd.name không phải là một chuỗi`);
                continue;
            }
...
const guildonlycounter = new Map();
    client.on("ready", () => {
        console.log(`Người dùng Bot ${client.user.username} đã được đăng nhập và sẵn sàng sử dụng!`.brightYellow);
        setInterval(() => {
    ...
    .on("playSong", async (message, queue, song) => {
       ...
       try{queue.connection.voice.setDeaf(true);}catch (error) {
        console.error(error)
        functions.embedbuilder(client, 5000, message, config.colors.yes, "ERROR: ", "```" + error.toString().substr(0, 100) + "```" + "\n\n**Error got sent to my owner!**")
    ...
    .on("playList", (message, queue, playlist, song) => {
        try{queue.connection.voice.setDeaf(true);}catch (error) {
            console.error(error)
            functions.embedbuilder(5000, message, config.colors.yes, "ERROR: ", "```" + error.toString().substr(0, 100) + "```" + "\n\n**Error got sent to my owner!**")
            functions.errorbuilder(error.stack.toString().substr(0, 2000))
           }
           try{queue.connection.voice.setSelfDeaf(true);}catch (error) {
            console.error(error)
