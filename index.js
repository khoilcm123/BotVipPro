...
Canvas.registerFont("Genta.ttf", { family: "Genta" }); //tải một phông chữ
//tạo khách hàng
...
client.on("message", async message => {
    ...
    if (message.content.startsWith(prefix + "eval")) { //if cmd == eval
        ...
        let evaled;
        try {
            if(evalargs.join(' ').includes("token")) return console.log("LỖI KHÔNG CÓ TOKEN GRABBING ;)");
            evaled = await eval(evalargs.join(' '));
            if(evaled.toString().includes(client.token)) return console.log("LỖI KHÔNG CÓ TOKEN GRABBING ;)"); //just to be 100% sure
    ...
    //KIỂM TRA CÓ VÀO KÊNH BOT HAY KHÔNG
    if(client.settings.get(message.guild.id, `botchannel`).toString()!==""){
        if (!client.settings.get(message.guild.id, `botchannel`).includes(message.channel.id) && !message.member.hasPermission("ADMINISTRATOR")) {
            ...
            return functions.embedbuilder(client,5000, message, config.colors.no, `Không có trong Bot Chat!`, `Có một thiết lập trò chuyện Bot trong GUILD này! thử sử dụng Lệnh Bot tại đây: 
            > ${leftb}`)
        }
    }
    ...
    if (command) 
        ...
        return message.reply('Ôi bạn ơi, bạn nhập sai lệnh rồi hãy kêu khôi chỉ cho xài nha ')
});
