...
module.exports = {
  ...
  run: async (client, message, args) => {
     ...
     if (!message.member.voice.channel) return embedbuilder(5000, message, config.colors.no, "`" + message.author.tag + "`"  + "❌ \`Bạn phải tham gia Kênh voice nào đó\`")
     if (message.member.voice.channel.id !== message.member.guild.me.voice.channel.id) return embedbuilder(5000, message, config.colors.no, "`" + message.author.tag + "`"  + "❌ \`Bạn phải tham gia Kênh voice của tôi\`")
