...
module.exports = {
    ...
    playsongyes: async function(client, message, queue, song) {
        try {
            ...
            .addField("♾ Vòng:", `>>> ${queue.repeatMode ? queue.repeatMode === 2 ? "<:YES:805375191739662366> Queue" : "<:YES:805375191739662366> Song" : "<:NO:805375191819616266>"}`, true)
            .addField("↪️ Tự chạy:", `>>> ${queue.autoplay ? "<:YES:805375191739662366>" : "<:NO:805375191819616266>"}`, true)
            .addField("❔ Bộ lọc:", `>>> ${queue.filter || "<:NO:805375191819616266>"}`, true)
            .addField("🎧 Vai trò DJ:", `>>> ${message.guild.roles.cache.get(client.settings.get(message.guild.id, `djrole`)) ? message.guild.roles.cache.get(client.settings.get(message.guild.id, `djrole`)) : "\`No Dj-Role setu+\`"}`, true)
            .setFooter(client.user.username, client.user.displayAvatarURL())
